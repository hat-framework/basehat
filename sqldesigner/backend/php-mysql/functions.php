...
function setup_saveloadlist() {
                $file = '../../../../../recursos/database/config/conection.php';
                if(file_exists($file)) {
                    require_once $file;
                    $server = bd_server;
                    $user   = bd_user;
                ...
                else{ die('Arquivo de conexão como o banco de dados não encontrado!');}
                $table  = "wwwsqldesigner";
		if(!defined('SERVER'))   define("SERVER"  ,$server);
	...
	function setup_import() {
                if(!defined('TABLE')) define("TABLE"   ,'information_schema');
                setup_saveloadlist();
	}
	function connect() {
