<?php
	...
	function connect(&$sqliteerror) {
            $initReq = false;
            if( !file_exists( FILE ) ) $initReq = true;
                    $GLOBALS['sqlConnect'] = sqlite_open(  FILE , 0666, $sqliteerror);
	...
	switch( $a ) {
		case 'list' :
			setup_saveloadlist();
			if( !connect($sqliteerror) ) {
				header( 'HTTP/1.0 503 Service Unavailable -- ' . $sqliteerror);
				break;
			}
