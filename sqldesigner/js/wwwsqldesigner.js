...
SQL.TableManager.prototype.click = function(e) { /* finish adding new table */
	...
	if (this.adding) {
		...
		var y = e.clientY + scroll[1];
                ...
                newtable = this.owner.addTable("Action",x,y);
                var k = newtable.addKey("PRIMARY","");
                if(SQL.Designer.getOption("db") == "sequence"){
                    newtable.addRow("Página"   ,{ai:false});
                    newtable.addRow("Título"   ,{ai:false});
                    newtable.addRow("Descrição",{ai:false});
                    newtable.addRow("Permissão",{ai:false});
                }
		else {
                    var r = newtable.addRow("cod",{ai:true});
                    k.addRow(r);
                }
	}
